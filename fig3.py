...
def main(omg):
    ...
    tau_list = np.linspace(0, 1000, 1000)
    corr_list = qt.correlation_2op_1t(H0, rho_ss, tau_list, [La, Ls], X_dot_m, X_dot_p, reverse=True)
    ker_list = corr_list * np.exp(1j * omg * tau_list)
    return np.abs(2 * integrate.simpson(ker_list, x=tau_list))
